const { getStoredIdOrGenerateId, getCoordinates, getMetaData, getTimestamp } = require('../utils/index.js');
...
const track_sessions = function () {
  ...
  const track_user_inactivity = function () {
    timerEnd = setInterval(function () {
      inactivityCounter++;
      if (inactivityCounter >= inactivityTime) {
        console.log('You have been inactive for ' + inactivityCounter * 3 + ' seconds.' + sessionExpire);
        clearInterval(timer);
        end_session();
      }
    }, 5000);
